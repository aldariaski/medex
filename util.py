...
def sorted_merge_posts_and_tfs(posts_tfs1, posts_tfs2):
    ...
    while index1 < len(posts_tfs1) and index2 < len(posts_tfs2):
        if posts_tfs2[index2] > posts_tfs1[index1]:
            sorted_intersect.append(posts_tfs1[index1])
            index1 = index1 + 1
        elif posts_tfs1[index1] > posts_tfs2[index2]:
            sorted_intersect.append(posts_tfs1[index2])
            index2 = index2 + 1
        else:
            sorted_intersect.append(posts_tfs2[index1][0], posts_tfs1[index1][1] + posts_tfs2[index2][1])
            index1 = index1 + 1
            index2 = index2 + 1
    ...
    while (index2 < len(posts_tfs2)):
        sorted_intersect.append(posts_tfs2[index2])
        index2 = index2 + 1

    return sorted_intersect
